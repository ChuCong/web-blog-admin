    ...
    <div>
        ...
        <Content>
            <div class="flex justify-between mb-4">
                ...
                <button class="px-2 bg-blue-500 text-white rounded" @click="exportExcel">Xuất danh sách người
                    dùng</button>
            </div>
            <DataTable :itemHeader="Header" :itemValue="users" :paginate="paginate" @getPage="getPage"
                @sort="onSortChange">
...
const Header = [
    ...
    { text: "Số khóa học tham gia", value: "course_count", align: "left", width: 150, order: true },
    { text: "Số khóa học hoàn thành", value: "completed_courses_count", align: "left", width: 150, order: true },
    { text: "Lần học cuối cùng", value: "userlog_max_created_at", align: "left", width: 150, order: true },
...
const breadcrumb = [
    { name: "Danh sách người dùng", link: '/user/list' }
];
...
const exportExcel = async () => {
    loading.setLoading(true)
    const response = await store.exportExcel();
    loading.setLoading(false)
    if (!response) {
        messageError("Xuất danh sách người dùng thất bại!")
    }
}
</script>